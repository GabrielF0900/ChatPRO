  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (axios.isAxiosError(error) && error.response) {
        setError(error.response.data.erro || error.response.data.message || 'Erro ao criar conta');
      } else {
      ...
      <div className="bg-white w-full max-w-md p-8 rounded-xl shadow-md">
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <div>
            ...
            <div className="flex items-center border border-gray-300 rounded-lg px-3">
              ...
              <input
                ...
                required
                disabled={loading}
                className="flex-1 py-2 bg-transparent focus:outline-none disabled:bg-gray-100"
              />
            </div>
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-blue-600 hover:bg-blue-700 text-white py-2 rounded-lg font-medium transition disabled:bg-blue-400 disabled:cursor-not-allowed"
          >
