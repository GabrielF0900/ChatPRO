...
export default function ChatPage() {
  ...
  const [message, setM            </button>ge] = useState("");
  ...
  useEffect(() => {
    ...
    if (!userEmail || userEmail.startsWith('anonimo_')) {
      console.log('⚠️ UserEmail não disponível ou inválido, aguardando...');
      return;
    }
    
    // Se já existe uma conexão ativa para este usuário, não criar outra
    ...
    const connectWebSocket = () => {
      try {
        ...
        ws.onclose = () => {
          ...
          // Tentar reconectar após 5 segundos apenas se não foi fechado intencionalmente
          if (wsRef.current === ws) {
    ...
    // Aguardar um pouco antes de conectar para evitar reconexões muito rápidas
  ...
  }, [userEmail]); // Manter userEmail como dependência, mas agora com proteção melhorada
        ...
        <header className="flex justify-between items-center px-6 py-4 border-b border-gray-200 bg-white">
          ...
          <div className="flex items-center gap-4">
            <div className="text-right">
              <p className="text-sm font-medium text-gray-900">Bem-vindo, {currentUser.name || 'Usuário'}!</p>
            ...
            <button 
              onClick={() => {
                ...
                // Fechar conexão WebSocket antes de sair
                if (wsRef.current) {
                  wsRef.current.close();
                  wsRef.current = null;
                }
