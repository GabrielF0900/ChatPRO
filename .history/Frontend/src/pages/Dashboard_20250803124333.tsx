...
export default function ChatPage() {
  ...
  const userEmail = currentUser.email || `anonimo_${Date.now()}`;
        ...
        ws.onmessage = async (event) => {
          ...
          try {
            ...
            if (event.data instanceof Blob) {
              ...
              console.log('📄 Mensagem convertida do Blob:', messageData);
            } else {
              messageData = event.data;
              console.log('📄 Mensagem string direta:', messageData);
            }
            
            const data = JSON.parse(messageData);
            console.log('📋 Dados parseados no frontend:', data);
            
            // Verificar se a mensagem já foi processada
            if (data.id && processedMessageIds.current.has(data.id)) {
              console.log('⚠️ Mensagem já processada, ignorando:', data.id);
              return;
            }
            
            // Ignorar mensagens enviadas pelo próprio usuário
            if (data.senderId === userEmail) {
              console.log('⚠️ Mensagem do próprio usuário, ignorando:', data.senderId);
              return;
            }
            ...
            setMessages(prev => {
              ...
              console.log('📊 Novo estado das mensagens:', newState.length);
              return newState;
            });
          } catch (error) {
